...
<html>
  ...
  <body> 
		 <h1 align="center" style="font-family:华义彩云;"> 客户信息添加</h1>
			<form action="updateCustomerInfo.action" method="post" >
				      ...
				      <td>
					      <select name="customerPropertiesId">
					              <s:iterator value="#request.allCustomerProperties" id="CustomerProperties">
					                  <s:if test="#CustomerProperties.sjzdxxmc !='' && #CustomerProperties.enable=='yes'">
						                  <option value="${CustomerProperties.sjzdxxId }">
						                       ${CustomerProperties.sjzdxxmc}
						                  </option>
					                  </s:if>
					              </s:iterator>
					              <!--
					      ...
					      <td>
						      <select name="customerTypeId">
						         <s:iterator value="#request.allCustomerType" id="CustomerType">
					                  <s:if test="#CustomerType.sjzdxxmc !='' && #CustomerType.enable=='yes'">
						                  <option value="${CustomerType.sjzdxxId }">
						                       ${CustomerType.sjzdxxmc}
						                  </option>
					                  </s:if>
					              </s:iterator>
						      <!--  
					      ...
					      <td>
						      <select name="openBankId">
						              <s:iterator value="#request.allCustomerOpenBank" id="CustomerOpenBank">
					                    <s:if test="#CustomerOpenBank.sjzdxxmc !='' && #CustomerOpenBank.enable=='yes'">
						                  <option value="${CustomerOpenBank.sjzdxxId }">
						                       ${CustomerOpenBank.sjzdxxmc}
						                  </option>
					                    </s:if>
					                  </s:iterator>
					                  <!-- 
					      ...
					      <td>
						      <select name="customerSourceId">
						              <s:iterator value="#request.allCustomerSource" id="CustomerSource">
					                    <s:if test="#CustomerSource.sjzdxxmc !='' && #CustomerSource.enable=='yes'">
						                  <option value="${CustomerSource.sjzdxxId }">
						                       ${CustomerSource.sjzdxxmc}
						                  </option>
					                    </s:if>
					                  </s:iterator>
					                  <!--  
